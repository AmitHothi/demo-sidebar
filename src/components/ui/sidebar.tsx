...
const Sidebar = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(
  ({ className, children }, ref) => {
    ...
    const sidebar = (
      <div
        ref={ref}
        className={cn("flex h-full flex-col border-0 shadow-sm bg-zinc-700 dark:bg-gray-900", className)}
      >
        {children}
      </div>
    )
    ...
    if (isMobile) {
      ...
      //   <aside className="relative h-full inset-y-0 left-0 z-10 w-[--sidebar-width] transition-all duration-300 ease-in-out md:block [[data-sidebar=closed]_&]:left-[calc(var(--sidebar-width)*-1)]">
      //   {sidebar}
      // </aside>
      )
    }
    ...
    return (
      <aside className="fixed h-full inset-y-0 left-0 z-10 w-[--sidebar-width] transition-all duration-300 ease-in-out md:block [[data-sidebar=closed]_&]:left-[calc(var(--sidebar-width)*-1)]">
        {sidebar}
      </aside>
    )
  }
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cn("flex justify-center items-center border-t px-2.5 py-2", className)}
      {...props}
    />
  )
})
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cn(
        "px-1.5 text-xs font-medium text-gray-400",
        className
      )}
      {...props}
    />
